...
ledcSetup(rrpwm_motor_channel, PWM_FREQ, PWM_RESOLUTION);
  ...
  ledcAttachPin(MOTOR_right_RPWM, rrpwm_motor_channel);
  
  ledcSetup(rlpwm_motor_channel, PWM_FREQ, PWM_RESOLUTION);

  ledcAttachPin(MOTOR_right_LPWM, rlpwm_motor_channel);
  
  ledcSetup(lrpwm_motor_channel, PWM_FREQ, PWM_RESOLUTION);

  ledcAttachPin(MOTOR_left_RPWM, lrpwm_motor_channel);
    ledcSetup(llpwm_motor_channel, PWM_FREQ, PWM_RESOLUTION);

  ledcAttachPin(MOTOR_left_LPWM, llpwm_motor_channel);
...
u32 velocity_to_pwm(f32 velocity, bool *direction) {
  ...
  f32 pwm_f = (std::fabs(velocity)*60.0)/(2.0f*M_PI); // Requires #include <math.h>
  // 3. Clamp to max PWM
  if (pwm_f > PWM_MAX) {
    pwm_f = PWM_MAX;
  }
...
void motors(RobotData *robot_t) {
  ...
  ///////////////////////////////////////
      // robot_t->r_speed_pwm = 250;
    // robot_t->l_speed_pwm = 250;
  setMotor(rrpwm_motor_channel, rlpwm_motor_channel, direction_r,(uint32_t )robot_t->r_speed_pwm);
  setMotor(lrpwm_motor_channel, llpwm_motor_channel, direction_l,(uint32_t )robot_t->l_speed_pwm);
  Serial.print("rspeed: ");
  ...
  Serial.print(" ldiretion: ");
  Serial.println(direction_l);

    Serial.print("rspeed_pwm: ");

  Serial.print(robot_t->r_speed_pwm);
  Serial.print(" lspeed_pwm: ");
  Serial.println(robot_t->l_speed_pwm);
...
// check for changing defining
// void fork(RobotData *robot_t){
//   if (robot_t->fork_state == NORMAL) {
//     setMotor(rlpwm_motor_channel, lpwm_motor_channel, FORWARD, 100);
//   } else if (robot_t->fork_state == UP) {
//     setMotor(rlpwm_motor_channel, lpwm_motor_channel, FORWARD, 200);
//     setMotor(rpwm_motor_channel, rlpwm_motor_channel, FORWARD, 200);
//   } else if (robot_t->fork_state == DOWN) {
//     setMotor(rlpwm_motor_channel, lpwm_motor_channel, BACKWARD, 50);
//     setMotor(rpwm_motor_channel, rlpwm_motor_channel, BACKWARD, 50);
//   }
// }
